...
def sub_eval_PTR_map(arg,q_pid,gls):
    ...
    for i in range(arg.shape[0]):
        # print(i,arg[i],len(gls))
        if q_pid in gls[arg[i]]:
            num+=1
            loss+=1.0*num/(i+1)
    # print(q_pid,loss,num)
...
def sub_eval_tds(arg,idx_from_gfs,paths):
    num=len(paths)
    m=0
    for j in range(num):
        paths[j]=sorted(paths[j])
        flag=False
        for i in range(topk):
            temp=idx_from_gfs[arg[i]]
            temp=sorted(temp)
            if temp==paths[j]:
                m+=1
                break
...
def eval_tds(args,idx_from_gfs,qls,paths,idx2pathidx,tpath2idx): 
    m=0
    num_q=args.shape[0]
    for i in range(num_q):
        arg=args[i,:]
        q_pid=qls[i]
        sub_paths_idx=idx2pathidx[q_pid]
        sub_paths=[tpath2idx[j] for j in sub_paths_idx]
        m+=sub_eval_tds(arg,idx_from_gfs,sub_paths)  
    m/=num_q
    return num_q
...
def tds2(distmat,qls,idx2pathidx,tpath2index,cluster2index,topk=20,threshold=100,indices=None):
...
def tc(distmat,qls,tls,idx2pathidx,tpath2index,cluster2index,topk=20,threshold=100,indices=None):
    temp=0
    for j in range(len(cluster2index)):         
        path=cluster2index[j]
        path_labels=[tls[j] for j in path]
